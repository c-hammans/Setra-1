  ...
  return <div className={`share-card share-format-${options.format} share-background-${options.background} share-tone-${options.textTone}`} style={style}>
    ...
    <div className="share-information-layer">
      ...
      {options.format==="sticker"
        ?<CompactShareSticker data={data}/>
        :data.kind==="pb"
          ?<PBShareCard data={data}/>
          :data.kind==="award"
            ?<AwardShareCard data={data}/>
...
export function AwardShareCard({data}:{data:ShareCardData}){return <div className="share-pb-content share-award-content"><small>AWARD EARNED</small><strong>{data.result}</strong><span>{data.title}</span>{data.date?<p>{new Intl.DateTimeFormat("en-AU",{day:"numeric",month:"short",year:"numeric"}).format(new Date(`${data.date}T12:00:00`))}</p>:null}</div>}
