...
const sportNames:Record<ShareCardData["sport"],string>={strength:"STR",run:"RUN",bike:"RIDE",swim:"SWIM",row:"ROW",walk_hike:"WALK",elliptical:"ELLIP",cross_training:"CROSS",custom:"END"};

function ShareBrand(){return <span className="share-brand"><i/><b>setra</b></span>}
function SportGlyph({sport}:{sport:ShareCardData["sport"]}){return <span className={`share-sport-glyph share-sport-${sport}`} aria-hidden="true">{sportNames[sport]}</span>}
...
export function WorkoutShareCard({data}:{data:ShareCardData}){return <div className="share-workout-content"><SportGlyph sport={data.sport}/><div className="share-result"><small>{data.title}</small><strong>{data.result}</strong>{data.secondary?<span>{data.secondary}</span>:null}</div><div className="share-metrics">{data.metrics.map(metric=><p key={`${metric.label}-${metric.value}`}><b>{metric.value}</b><span>{metric.label}</span></p>)}</div>{data.date?<time>{new Intl.DateTimeFormat("en-AU",{day:"numeric",month:"short",year:"numeric"}).format(new Date(`${data.date}T12:00:00`))}</time>:null}</div>}

export function PBShareCard({data}:{data:ShareCardData}){return <div className="share-pb-content"><SportGlyph sport={data.sport}/><small>{data.title}</small><strong>{data.result}</strong>{data.secondary?<span>{data.secondary}</span>:null}{data.improvement?<b>{data.improvement}</b>:null}{data.previous?<p>{data.previous}</p>:null}</div>}

export function CompactShareSticker({data}:{data:ShareCardData}){const strengthHero=data.kind==="workout"&&data.sport==="strength"?data.metrics[0]?.value:data.result;const metrics=data.kind==="workout"&&data.sport==="strength"?data.metrics.slice(1,3):data.metrics.slice(0,2);return <div className="share-sticker-content"><SportGlyph sport={data.sport}/><div><small>{data.title}</small><strong>{strengthHero||data.result}</strong>{data.secondary?<span>{data.secondary}</span>:null}</div><div className="share-sticker-metrics">{metrics.map(metric=><p key={`${metric.label}-${metric.value}`}><b>{metric.value}</b><span>{metric.label}</span></p>)}</div></div>}
