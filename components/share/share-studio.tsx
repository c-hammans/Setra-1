...
export function ShareStudio({items,accent,onClose,footer}:Props){
  ...
  async function save(){try{const blob=await image();const url=URL.createObjectURL(blob);const link=document.createElement("a");link.href=url;link.download=`setra-${data.kind}-${data.id}.png`;link.click();setTimeout(()=>URL.revokeObjectURL(url),1000);setMessage("Saved as a high-resolution PNG.")}catch{setMessage("The image could not be saved. Please try again.")}}
  async function share(){try{const blob=await image();const file=new File([blob],`setra-${data.kind}-${data.id}.png`,{type:"image/png"});if(navigator.share&&navigator.canShare?.({files:[file]}))await navigator.share({title:data.kind==="pb"?"My Setra PB":"My Setra workout",files:[file]});else await save()}catch(error){if((error as Error).name!=="AbortError")setMessage("Sharing is unavailable here. Use Save instead.")}}
