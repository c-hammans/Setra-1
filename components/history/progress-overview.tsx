...
import {addCalendarDays} from "@/lib/setra/week";

type ProgressWeek={key:string;label:string;sessions:number;strength:number;endurance:number;minutes:number;durationCoverage:number;distance:number;distanceCoverage:number};
...
export function ProgressOverview({weeks,showStrength,showEndurance,strengthTrends,enduranceTrends,preferredUnit,onExercise}:{weeks:ProgressWeek[];showStrength:boolean;showEndurance:boolean;strengthTrends:StrengthTrend[];enduranceTrends:EnduranceTrend[];preferredUnit:StrengthUnit;onExercise:(id:string)=>void}){
  ...
  const period=weeks.length?`${dateLabel(weeks[0].key)} – ${dateLabel(addCalendarDays(weeks.at(-1)!.key,6))}`:"No period";
  return <section className="progress-overview" aria-label="Training progress">
    ...
    <div className="weekly-progress-bars" role="group" aria-label={`Completed sessions by week from ${period}`}>
