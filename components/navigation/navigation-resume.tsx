...
export function NavigationResume(){
  ...
  useEffect(()=>{
    ...
    const save=()=>{const location=current();if(resumableLocation(location))localStorage.setItem(key,JSON.stringify({location,savedAt:Date.now()} satisfies SavedLocation))};
  ...
  useEffect(()=>{
    ...
    if(resumableLocation(location))localStorage.setItem(`setra-last-location:${user.id}`,JSON.stringify({location,savedAt:Date.now()} satisfies SavedLocation));
  },[pathname,user?.id]);
