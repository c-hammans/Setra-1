...
type Props={session:EnduranceSession;onClose:()=>void;onEdit:()=>void;onEditTemplate?:()=>void;onComplete?:()=>void;onShare?:()=>void;onToggleSkip?:()=>void;onDelete:()=>void};
...
export function EnduranceWorkoutView({session,onClose,onEdit,onEditTemplate,onComplete,onShare,onToggleSkip,onDelete}:Props){
  ...
  return <div className="overlay high-overlay" onMouseDown={event=>{if(event.target===event.currentTarget)onClose()}}><section className="sheet endurance-workout-view" onMouseDown={event=>event.stopPropagation()}><div className="sheet-handle"/><div className="sheet-title compact-endurance-title"><div><span>{activityLabel(session.activityType).toUpperCase()} · {session.status.toUpperCase()}</span><h2>{session.title}</h2></div><button onClick={onClose} aria-label="Close">×</button></div>
    ...
    <section className="workout-view-structure"><header><span>WORKOUT</span>{session.blocks.length>0&&<small>{session.blocks.length} structured {session.blocks.length===1?"item":"items"}</small>}</header><EnduranceStructureView session={session}/></section>
    {session.notes&&<section className="workout-view-notes"><span>NOTES</span><p>{session.notes}</p></section>}
    {session.status==="planned"&&<div className="future-device-slot"><span>DEVICE SYNC</span><small>Watch syncing is not available yet.</small></div>}
    <div className="endurance-detail-actions">{session.status==="planned"&&onComplete&&<button className="primary-button" onClick={onComplete}>Complete session <span>→</span></button>}{session.status==="completed"&&onShare&&<button className="share-completed-button" onClick={onShare}>Share activity <span>↗</span></button>}<button onClick={onEdit}>Edit {session.status==="completed"?"recorded activity":"this occurrence"}</button>{session.status==="planned"&&onEditTemplate&&<button onClick={onEditTemplate}>Edit reusable template</button>}{session.status==="planned"&&onToggleSkip&&<button onClick={onToggleSkip}>{session.skipped?"Restore this occurrence":"Skip this occurrence"}</button>}<button className="delete-workout-button" onClick={onDelete}>{session.status==="planned"?"Cancel this occurrence":"Delete recorded activity"}</button></div>
  </section></div>;
}
