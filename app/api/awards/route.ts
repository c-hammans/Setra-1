...
export async function POST(request:Request){
  try{
    ...
    const planByKey=new Map(plans.map(item=>[item.key,item]));const completions:{key:string;clientId:string;completedAt:string;status:"completed"|"partial"}[]=[];
    for(const row of workoutRows){if(!genuineStrengthSessionIds.has(String(row.training_session_id||"")))continue;const linked=Array.isArray(row.workout_templates)?row.workout_templates[0]:row.workout_templates;const exercises=(row.workout_exercises||[]) as Row[];const partial=exercises.some(exercise=>exercise.skipped||(exercise.workout_sets||[]).some((set:Row)=>!set.completed));if(linked?.client_id)completions.push({key:`strength:${linked.client_id}:${row.workout_date}`,clientId:String(row.client_id||row.id),completedAt:String(row.completed_at||new Date().toISOString()),status:partial?"partial":"completed"})}
    for(const row of completedRows){if(row.modality!=="endurance"||!row.planned_session_id)continue;const plannedClientId=plannedClientById.get(String(row.planned_session_id));if(plannedClientId)completions.push({key:`endurance:${plannedClientId}`,clientId:String(row.client_id||row.id),completedAt:String(row.completed_at||new Date().toISOString()),status:"completed"})}
    for(const completion of completions){const plan=planByKey.get(completion.key);if(!plan||plan.status===completion.status)continue;plan.status=completion.status;await admin.from("training_plan_occurrences").update({status:completion.status,linked_completion_client_id:completion.clientId,completed_at:completion.completedAt}).eq("user_id",user.id).eq("occurrence_key",completion.key)}
