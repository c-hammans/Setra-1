...
const defaults:ProfileSettings={displayName:"",fullName:"",dateOfBirth:"",preferredUnit:"kg",trainingGoal:"",experienceLevel:"",trainingPreference:"strength",appColour:"#409ECE",appearanceMode:"system",textScale:1,showWorkoutTimingPopup:true,showPbPopup:true,weekStartsOn:1,lastWeeklyPreviewWeekStart:null};
...
export default function ProfilePage(){
  ...
  return <main className="profile-screen" style={theme} data-light-accent={contrast==="#0F172A"} data-theme={resolvedAppearance}>
    ...
    <div className="profile-content">
      ...
      <section className="profile-section"><header><span>CALENDAR</span><h2>Training week</h2><p>Choose which day starts your calendar week and weekly summaries.</p></header><label className="profile-field">START OF WEEK<select value={settings.weekStartsOn} onChange={event=>setSettings({...settings,weekStartsOn:Number(event.target.value) as ProfileSettings["weekStartsOn"]})}>{weekdayOptions.map(([value,label])=><option value={value} key={value}>{label}</option>)}</select></label></section>
