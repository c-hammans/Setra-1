...
export default function AwardsPage(){
  ...
  useDialogFocusTrap(Boolean(selected||sharing));
  useEffect(()=>{if(!selected&&!sharing)return;const close=(event:KeyboardEvent)=>{if(event.key!=="Escape")return;if(sharing)setSharing(null);else setSelected(null)};window.addEventListener("keydown",close);return()=>window.removeEventListener("keydown",close)},[selected,sharing]);
  return <main className="awards-screen" style={theme} data-theme={resolved}>
    <header className="awards-header"><button onClick={()=>router.back()} aria-label="Back to Setra">‹</button><b>Awards</b><span/></header>
    <div className="awards-content">
      ...
      {error?<section className="awards-error"><b>Awards need one final setup step</b><p>{error}</p></section>:!data?<section className="awards-loading">Building your training record…</section>:<>
        ...
        <section className="streak-record"><header><span>CONSISTENCY</span><h2>Current and best</h2></header><div>{([['Active weeks',data.streaks.activeWeeks],['Training days',data.streaks.trainingDays],['Weekly goal',data.streaks.weeklyGoal],['Planned weeks',data.streaks.plannedWeeks],['Days with Setra',data.streaks.setraUse]] as const).map(([label,streak])=>label==="Weekly goal"&&!data.weeklySessionGoal?<article key={label}><span>{label}</span><b>No weekly goal set</b><small><Link href="/profile">Configure in Profile</Link></small></article>:<article key={label}><span>{label}</span><b>{streak.current} current</b><small>{streak.longest} best</small></article>)}</div><p>Rest days do not affect Active Weeks. Training Days is a separate literal day-to-day record.</p></section>
