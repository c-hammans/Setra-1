...
const hasCompletedStrengthWork=(workout:Workout)=>completedExercises(workout).length>0;
    ...
    <main className="app-shell" data-light-accent={contrastColour(appColour)==="#0F172A"} data-theme={resolvedAppearance} style={createSetraTheme(appColour,resolvedAppearance,textScale)}>
      ...
      <section className={`content content-${tab}`}>
        {tab === "today" && <>
          ...
          {((showStrength&&completedWorkouts.length>0)||(showEndurance&&completedEndurance.length>0))&&<section className="completed-today"><span className="completed-heading">{selectedDate===today?"COMPLETED TODAY":"COMPLETED"}</span>{showEndurance&&completedEndurance.map(session=><button key={session.id} className="completed-card endurance-diary-card" onClick={()=>setEnduranceDetailId(session.id)}><ActivityIcon type={session.activityType}/><span><b>{session.title}</b><small>{enduranceSummary(session)}</small></span><em>View ›</em></button>)}{showStrength&&completedWorkouts.map(workout=>{const exercises=completedExercises(workout);return <button key={workout.id} className="completed-card" onClick={()=>setDetailId(workout.id)}><span className="completed-check">✓</span><span><b>{workout.name}</b><small>{exercises.length} exercises · {exercises.reduce((sum,exercise)=>sum+completedSets(exercise).length,0)} sets completed</small></span><em>View ›</em></button>})}</section>}
          ...
          <div className="recent-training-list">{recentHistory.map(item=>item.kind==="strength"?<button className="recent-card" key={`recent-strength-${item.id}`} onClick={()=>setDetailId(item.id)}><span className="recent-date"><b>{new Date(`${item.date}T12:00:00`).getDate()}</b>{new Intl.DateTimeFormat("en",{month:"short"}).format(new Date(`${item.date}T12:00:00`))}</span><span className="recent-main"><b>{item.title}</b><small>{isHybrid?"Strength · ":""}{completedExercises(item.workout).length} exercises</small></span><span className="chevron">›</span></button>:<button className="recent-card" key={`recent-endurance-${item.id}`} onClick={()=>setEnduranceDetailId(item.id)}><ActivityIcon type={item.session.activityType}/><span className="recent-main"><b>{item.title}</b><small>{activityLabel(item.session.activityType)} · {enduranceSummary(item.session)}</small></span><span className="chevron">›</span></button>)}</div>
        </>}
