    ...
    <main className="app-shell" data-light-accent={lightAppColours.has(appColour)} style={{"--accent":appColour,"--accent-contrast":lightAppColours.has(appColour)?"#0F172A":"#FFFFFF","--accent-soft":mixHex(appColour,"#FFFFFF",.7),"--accent-ink":mixHex(appColour,"#0F172A",.72),"--accent-tint":mixHex(appColour,"#FFFFFF",.11),"--accent-border":mixHex(appColour,"#FFFFFF",.34)} as CSSProperties}>
      ...
      {active && liveEditIndex !== null && active.exercises[liveEditIndex] && <div className="overlay high-overlay" onMouseDown={()=>setLiveEditIndex(null)}>
        <section className="sheet live-edit-sheet" onMouseDown={e=>e.stopPropagation()}>
          ...
          <label className="search live-swap-search"><span>⌕</span><input autoFocus value={liveSwapQuery} onChange={event=>setLiveSwapQuery(event.target.value)} placeholder="Search exercise, equipment or muscle"/></label>
