...
export default function Home() {
  ...
  const prominentLayerOpen=Boolean(active||editor||picker||scheduleTemplateId||finishDialogOpen||newPBs.length||completedShare||completedEnduranceShare||detailId||deleteWorkoutId||deleteTemplateId||liveEditIndex!==null||liveAddOpen||exerciseHistoryId||feedbackOpen||trainingAction||enduranceEditor||enduranceDetailId||deleteEnduranceId);
  ...
  function saveEnduranceSession(session:EnduranceSession){const newlyCompleted=session.status==="completed"&&enduranceEditor?.mode==="log"&&enduranceEditor.initial?.status!=="completed";setEnduranceSessions(current=>[session,...current.filter(item=>item.id!==session.id)].sort((a,b)=>b.date.localeCompare(a.date)));runTrainingCloud(service=>service.save(session));setSelectedDate(session.date);setEnduranceEditor(null);setEnduranceDetailId(newlyCompleted?null:session.status==="completed"?session.id:null);if(newlyCompleted)setCompletedEnduranceShare(session)}
    ...
    <main className="app-shell" data-light-accent={contrastColour(appColour)==="#0F172A"} data-theme={resolvedAppearance} style={createSetraTheme(appColour,resolvedAppearance,textScale)}>
      ...
      <section className={`content content-${tab}`}>
        {tab === "today" && <>
          ...
          {showEndurance&&<section className={`training-quick-actions preference-${trainingPreference}`}><span>QUICK ACTIONS</span><div><button className="quick-action-button" onClick={()=>isHybrid?setTrainingAction("plan"):setEnduranceEditor({mode:"plan"})}><NavIcon name="plan"/><span className="quick-action-copy"><b>Plan session</b><small>Build your next workout</small></span><em aria-hidden="true">›</em></button><button className="quick-action-button" onClick={()=>isHybrid?setTrainingAction("log"):setEnduranceEditor({mode:"log"})}><NavIcon name="today"/><span className="quick-action-copy"><b>Log activity</b><small>Record today’s training</small></span><em aria-hidden="true">›</em></button></div></section>}
        ...
        {tab === "plan" && <>
          ...
          {showEndurance&&enduranceSessions.some(session=>session.status==="planned"&&!fulfilledPlannedIds.has(session.id))&&<><div className="section-title plan-section-title"><div><span>ENDURANCE</span><h2>Planned sessions</h2></div></div><div className="endurance-plan-list">{enduranceSessions.filter(session=>session.status==="planned"&&!fulfilledPlannedIds.has(session.id)).map(session=><button key={session.id} onClick={()=>setEnduranceDetailId(session.id)}><span className="activity-pill">{activityShort(session.activityType)}</span><span><b>{session.title}</b><small>{formatDate(session.date)} · {enduranceSummary(session)}</small></span><em>›</em></button>)}</div></>}
      ...
      {completedShare&&newPBs.length===0&&<ShareStudio items={[strengthWorkoutShareData(completedShare)]} accent={appColour} onClose={()=>setCompletedShare(null)} footer={!completedShare.templateId?<div className="save-session-template"><span>SAVE FOR NEXT TIME</span><p>Turn this Add as I go session into a reusable workout.</p><input value={newTemplateName} disabled={sessionTemplateSaved} onChange={event=>setNewTemplateName(event.target.value)} maxLength={40} placeholder="Template name"/><button disabled={!newTemplateName.trim()||sessionTemplateSaved||completedShare.exercises.length===0} onClick={saveSessionAsTemplate}>{sessionTemplateSaved?"Added to templates ✓":"Add as template"}</button></div>:undefined}/>}
