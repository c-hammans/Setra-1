...
import type { AppData, EnduranceSession, EnduranceTemplate, Exercise, SetLog, Template, TemplateExercise, TrainingPreference, Workout, WorkoutExercise } from "@/lib/setra/types";
...
export default function Home() {
  ...
  useEffect(()=>{
    ...
    const timer=window.setTimeout(()=>{
      ...
      retryCloud(()=>diaryService.saveWorkout({...active,updatedAt:local.updatedAt},"in_progress",queued.revision),2).then(()=>{removePendingDiaryChange(queueKey,queued.operationId,user?.id);setDraftSaveState("saved")}).catch(()=>{setDraftSaveState(local.ok&&queued.ok?"pending":"error");setCloudMessage(local.ok&&queued.ok?"Saved on this device. Cloud sync will retry automatically when Setra reconnects.":"The workout draft could not be saved.")});
    },650);
    return()=>window.clearTimeout(timer);
  },[active,diaryService,editingWorkoutId,user?.id]);
  useEffect(()=>{if(!profileReady||!diaryService||!user?.id)return;let cancelled=false;let replaying=false;const replay=async()=>{if(replaying)return;replaying=true;try{for(const change of loadPendingDiaryChanges(user.id)){if(cancelled)return;try{if(change.kind==="save_workout")await diaryService.saveWorkout(change.payload.workout,change.payload.status,change.revision);else if(change.kind==="replace_schedule")await diaryService.replaceSchedule(change.payload.items,change.revision);else if(change.kind==="save_strength_template")await diaryService.saveTemplate(change.payload.template,change.revision);else if(change.kind==="save_endurance_session"){if(!trainingService)return;await trainingService.save(change.payload.session,change.revision)}else if(change.kind==="save_endurance_template"){if(!trainingService)return;await trainingService.saveTemplate(change.payload.template,change.revision)}else if(change.kind==="delete_workout")await diaryService.deleteWorkout(change.payload.clientId,change.revision);else if(change.kind==="delete_strength_template")await diaryService.deleteTemplate(change.payload.clientId,change.revision);else if(change.kind==="delete_endurance_session"){if(!trainingService)return;await trainingService.delete(change.payload.clientId,change.revision)}else{if(!trainingService)return;await trainingService.deleteTemplate(change.payload.clientId,change.revision)}removePendingDiaryChange(change.key,change.operationId,user.id)}catch(error){const normalized=normalizeWriteError(error);if(normalized.kind==="stale"){removePendingDiaryChange(change.key,change.operationId,user.id);continue}setCloudState("error");setCloudMessage(userWriteErrorMessage(normalized));break}}}finally{replaying=false}};void replay();const retryTimer=window.setInterval(()=>void replay(),15_000);const online=()=>void replay();const storage=(event:StorageEvent)=>{if(event.key===`form-pending-diary-changes:${user.id}`)void replay()};window.addEventListener("online",online);window.addEventListener("storage",storage);return()=>{cancelled=true;window.clearInterval(retryTimer);window.removeEventListener("online",online);window.removeEventListener("storage",storage)}},[profileReady,diaryService,trainingService,user?.id]);
  useEffect(()=>{
  ...
  function runCloud(action:(service:DiaryService)=>Promise<unknown>){
    ...
    setCloudState("loading");return retryCloud(()=>action(diaryService)).then(()=>{setCloudState("synced");setCloudMessage("");return true}).catch(error=>{analytics.track("save_failed",{outcome:"failed"});setCloudState("error");setCloudMessage(userWriteErrorMessage(error));return false});
  }
  function navigateTab(next:Tab){setTab(next);const url=next==="today"?"/":`/?tab=${next}`;window.history.pushState({},"",url)}
  function persistSchedule(items:AppData["scheduled"]){const key="schedule:current";const queued=queuePendingDiaryChange({key,kind:"replace_schedule",payload:{items}},user?.id);return runCloud(service=>service.replaceSchedule(items,queued.revision))?.then(saved=>{if(saved)removePendingDiaryChange(key,queued.operationId,user?.id);else if(queued.ok)setCloudMessage("Schedule changes are saved on this device and waiting to sync.");return saved})}
  function runTrainingCloud(action:(service:TrainingSessionService)=>Promise<unknown>){if(!trainingService)return;setCloudState("loading");return retryCloud(()=>action(trainingService)).then(()=>{setCloudState("synced");setCloudMessage("");return true}).catch(error=>{analytics.track("save_failed",{outcome:"failed"});setCloudState("error");setCloudMessage(userWriteErrorMessage(error));return false})}
  ...
  function saveEnduranceValue(value:EnduranceSession|EnduranceTemplate){
    if(!("status" in value)){
      ...
      runTrainingCloud(service=>service.saveTemplate(value,queued.revision))?.then(saved=>{if(saved)removePendingDiaryChange(key,queued.operationId,user?.id)});return;
    }
    ...
    runTrainingCloud(service=>service.save(session,queued.revision))?.then(saved=>{if(saved){removePendingDiaryChange(key,queued.operationId,user?.id);window.dispatchEvent(new Event("setra-training-changed"))}});
  }
  function deleteEnduranceSession(session:EnduranceSession){const key=`endurance-session:${session.id}`;const queued=queuePendingDiaryChange({key,kind:"delete_endurance_session",payload:{clientId:session.id}},user?.id);if(!queued.ok){setCloudState("error");setCloudMessage("The deletion could not be saved on this device, so the session was not removed.");return}setEnduranceSessions(current=>current.filter(item=>item.id!==session.id));runTrainingCloud(service=>service.delete(session.id,queued.revision))?.then(saved=>{if(saved)removePendingDiaryChange(key,queued.operationId,user?.id)});setEnduranceDetailId(null);setDeleteEnduranceId(null)}
  function deleteEnduranceTemplate(){if(!deleteEnduranceTemplateId)return;const clientId=deleteEnduranceTemplateId;const key=`endurance-template:${clientId}`;const queued=queuePendingDiaryChange({key,kind:"delete_endurance_template",payload:{clientId}},user?.id);if(!queued.ok){setCloudState("error");setCloudMessage("The deletion could not be saved on this device, so the template was not removed.");return}setEnduranceTemplates(current=>current.filter(item=>item.id!==clientId));runTrainingCloud(service=>service.deleteTemplate(clientId,queued.revision))?.then(saved=>{if(saved)removePendingDiaryChange(key,queued.operationId,user?.id)});setDeleteEnduranceTemplateId(null)}
  ...
  const previousWeekDates=useMemo(()=>{const anchor=new Date(`${currentWeekDates[0]}T12:00:00`);anchor.setDate(anchor.getDate()-7);return weekDateKeys(localDateKey(anchor),weekStartsOn)},[currentWeekDates,weekStartsOn]);
  const weeklyActuals=useMemo(()=>{const count=(dates:string[])=>{const strength=data.workouts.filter(workout=>workout.date>=dates[0]&&workout.date<=dates[6]&&hasCompletedStrengthWork(workout)).length;const endurance=enduranceSessions.filter(session=>session.status==="completed"&&session.date>=dates[0]&&session.date<=dates[6]).length;return{strength,endurance,total:strength+endurance}};return{current:count(currentWeekDates),previous:count(previousWeekDates)}},[currentWeekDates,data.workouts,enduranceSessions,previousWeekDates]);
  ...
  const previousSets = (exerciseId: string) => {const exercise=data.workouts.flatMap(workout=>workout.exercises).find(item=>item.exerciseId===exerciseId&&completedSets(item).length>0);return exercise?completedSets(exercise):[]};
  ...
  function saveDraft() {
    ...
    runCloud(service=>service.saveWorkout({...active,updatedAt:local.updatedAt},"in_progress",queued.revision))?.then(saved=>{if(saved)removePendingDiaryChange(key,queued.operationId,user?.id);setDraftSaveState(saved?"saved":local.ok&&queued.ok?"pending":"error")});
  ...
  function deleteCompletedWorkout() {
    ...
    runCloud(service=>service.deleteWorkout(clientId,queued.revision))?.then(saved=>{if(saved)removePendingDiaryChange(key,queued.operationId,user?.id)});
  ...
  function deleteTemplate() {
    ...
    runCloud(service=>service.deleteTemplate(clientId,queued.revision))?.then(saved=>{if(saved)removePendingDiaryChange(key,queued.operationId,user?.id)});
  ...
  function saveSessionAsTemplate() {
    ...
    const key=`strength-template:${template.id}`;setData(current=>({...current,templates:[...current.templates,template]}));const queued=queuePendingDiaryChange({key,kind:"save_strength_template",payload:{template}},user?.id);runCloud(service=>service.saveTemplate(template,queued.revision))?.then(saved=>{if(saved)removePendingDiaryChange(key,queued.operationId,user?.id)});setSessionTemplateSaved(true);
  }
  ...
  function saveTemplate() {
    ...
    runCloud(service=>service.saveTemplate(normalized,queued.revision))?.then(saved=>{if(saved)removePendingDiaryChange(key,queued.operationId,user?.id)});
  ...
  function scheduleEnduranceWorkout(){
    ...
    setEnduranceSessions(current=>[...additions,...current].sort((a,b)=>b.date.localeCompare(a.date)));additions.forEach(session=>{const key=`endurance-session:${session.id}`;const queued=queuePendingDiaryChange({key,kind:"save_endurance_session",payload:{session}},user?.id);runTrainingCloud(service=>service.save(session,queued.revision))?.then(saved=>{if(saved)removePendingDiaryChange(key,queued.operationId,user?.id)})});setSelectedDate(scheduleDate);setScheduleEnduranceTemplateId(null);
  }
    ...
    <main className="app-shell" data-light-accent={contrastColour(appColour)==="#0F172A"} data-theme={resolvedAppearance} style={createSetraTheme(appColour,resolvedAppearance,textScale)}>
      ...
      <section className={`content content-${tab}`}>
        {tab === "today" && <>
          ...
          <section className="weekly-actual-summary" aria-label={`Recorded training for ${compactDateRange(currentWeekDates)}`}><div><span>RECORDED · {compactDateRange(currentWeekDates)}</span><b>{weeklyActuals.current.total} {weeklyActuals.current.total===1?"session":"sessions"}</b><small>{weeklyActuals.current.strength} strength · {weeklyActuals.current.endurance} endurance</small></div><p><span>PREVIOUS WEEK</span><b>{weeklyActuals.previous.total}</b><small>recorded sessions</small></p></section>
        ...
        {tab === "plan" && <>
          ...
          {showEndurance&&enduranceSessions.some(session=>session.status==="planned"&&!fulfilledPlannedIds.has(session.id))&&<><div className="section-title plan-section-title"><div><span>ENDURANCE</span><h2>Planned sessions</h2></div></div><div className="endurance-plan-list">{enduranceSessions.filter(session=>session.status==="planned"&&!fulfilledPlannedIds.has(session.id)).map(session=><button key={session.id} onClick={()=>setEnduranceDetailId(session.id)}><ActivityIcon type={session.activityType}/><span><b>{session.title}</b><small>{formatDate(session.date)} · {enduranceSummary(session)}</small></span><em>›</em></button>)}</div></>}
