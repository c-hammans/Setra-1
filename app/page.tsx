...
import {formatTrainingStep} from "@/lib/setra/endurance-steps";
...
function EnduranceBlockDisplay({session,detail=false}:{session:EnduranceSession;detail?:boolean}){
  const topLevel=session.blocks.filter(block=>!block.parentId);
  return <>{topLevel.map((block,index)=>{
    const children=block.type==="repeat_group"?session.blocks.filter(child=>child.parentId===block.id):[];
    if(detail)return <div className={block.type==="repeat_group"?"endurance-repeat-display":""} key={block.id}><small>{block.type==="repeat_group"?`GROUP ${index+1}`:`STEP ${index+1}`}</small><b>{block.title||block.type}</b><strong>{formatTrainingStep(block,session.activityType)}{children.length?` · ${children.length} ${children.length===1?"step":"steps"}`:""}</strong>{block.instructions&&<p>{block.instructions}</p>}{children.length>0&&<div className="endurance-repeat-children">{children.map((child,childIndex)=><div key={child.id}><small>STEP {index+1}.{childIndex+1}</small><b>{child.title||child.type}</b><strong>{formatTrainingStep(child,session.activityType)}</strong>{child.instructions&&<p>{child.instructions}</p>}</div>)}</div>}</div>;
    return <span className={block.type==="repeat_group"?"endurance-repeat-preview":""} key={block.id}><b>{index+1}. {block.title||block.type}</b>{formatTrainingStep(block,session.activityType)}{children.length>0&&<em>{children.map((child,childIndex)=><small key={child.id}>{index+1}.{childIndex+1} {child.title||child.type} · {formatTrainingStep(child,session.activityType)}</small>)}</em>}{block.instructions&&<small>{block.instructions}</small>}</span>;
  })}</>;
}
...
export default function Home() {
  ...
  const [enduranceEditor,setEnduranceEditor]=useState<{mode:"plan"|"log";initial?:EnduranceSession|null}|null>(null);
  ...
  const prominentLayerOpen=Boolean(active||editor||picker||scheduleTemplateId||finishDialogOpen||newPBs.length||completedShare||detailId||deleteWorkoutId||deleteTemplateId||liveEditIndex!==null||liveAddOpen||exerciseHistoryId||feedbackOpen||trainingAction||enduranceEditor||enduranceDetailId||deleteEnduranceId);
  ...
  useEffect(()=>{if(!loaded||!trainingService)return;let cancelled=false;retryCloud(()=>trainingService.loadEndurance()).then(sessions=>{if(!cancelled)setEnduranceSessions(sessions)}).catch(error=>{if(!cancelled){setCloudState("error");setCloudMessage(error instanceof Error?error.message:"Endurance sessions could not be loaded. Your local copy is still safe.")}});return()=>{cancelled=true}},[loaded,trainingService]);
  ...
  function runTrainingCloud(action:(service:TrainingSessionService)=>Promise<unknown>){if(!trainingService)return;setCloudState("loading");retryCloud(()=>action(trainingService)).then(()=>{setCloudState("synced");setCloudMessage("")}).catch(error=>{setCloudState("error");setCloudMessage(error instanceof Error?error.message:"Activity save failed. Your local copy is still safe.")})}
  function saveEnduranceSession(session:EnduranceSession){setEnduranceSessions(current=>[session,...current.filter(item=>item.id!==session.id)].sort((a,b)=>b.date.localeCompare(a.date)));runTrainingCloud(service=>service.save(session));setSelectedDate(session.date);setEnduranceEditor(null);setEnduranceDetailId(session.status==="completed"?session.id:null);}
  ...
  const plannedEndurance=enduranceSessions.filter(session=>session.date===selectedDate&&session.status==="planned");
  ...
  const combinedHistory=useMemo(()=>[
    ...data.workouts.map(workout=>({kind:"strength" as const,date:workout.date,id:workout.id,title:workout.name,workout})),
  ...
  function saveWorkout(timing?:{startedAt:string;endedAt:string}) {
    ...
    if (!editingWorkoutId) {
      ...
      if (records.length&&showPbPopup) { setNewPBs(records); setPbTileStyle("colour"); setPbColour(appColour); setPbGraphic(0); setSelectedPBIndex(0); setPbShareMessage(""); }
    }
    if (timing && !editingWorkoutId) {
      setCompletedShare(completed);
      setSessionShareMessage("");
      setSessionShareColour(appColour);
      setNewTemplateName("");
  ...
  function deleteCompletedWorkout() {
    ...
    runCloud(service=>service.deleteWorkout(deleteWorkoutId));
  ...
  function saveSessionAsTemplate() {
    ...
    const template:Template={id:`template-${Date.now()}`,name:newTemplateName.trim(),focus:"Saved from Add as I go",color:"#409ECE",icon:"◆",warmup:(completedShare.warmup||[]).map(item=>({id:item.id,kind:item.kind,exerciseId:item.exerciseId,title:item.title,instructions:item.instructions})),exercises,supersetNames:completedShare.supersetNames};
    ...
    <main className="app-shell" data-light-accent={contrastColour(appColour)==="#0F172A"} data-theme={resolvedAppearance} style={{"--accent":appColour,"--accent-contrast":contrastColour(appColour),"--accent-soft":mixHex(appColour,"#FFFFFF",resolvedAppearance==="dark"?.78:.7),"--accent-ink":resolvedAppearance==="dark"?mixHex(appColour,"#FFFFFF",.76):mixHex(appColour,"#0F172A",.72),"--accent-tint":mixHex(appColour,resolvedAppearance==="dark"?"#10151D":"#FFFFFF",resolvedAppearance==="dark"?.18:.11),"--accent-border":mixHex(appColour,resolvedAppearance==="dark"?"#10151D":"#FFFFFF",resolvedAppearance==="dark"?.5:.34),"--text-scale-percent":textScalePercent(textScale),"--text-scale-number":textScale} as CSSProperties}>
      ...
      <section className="content">
        {tab === "today" && <>
          ...
          {((showStrength&&completedWorkouts.length>0)||(showEndurance&&completedEndurance.length>0))&&<section className="completed-today"><span className="completed-heading">{selectedDate===today?"COMPLETED TODAY":"COMPLETED"}</span>{showEndurance&&completedEndurance.map(session=><button key={session.id} className="completed-card endurance-diary-card" onClick={()=>setEnduranceDetailId(session.id)}><span className="activity-pill">{activityShort(session.activityType)}</span><span><b>{session.title}</b><small>{enduranceSummary(session)}</small></span><em>View ›</em></button>)}{showStrength&&completedWorkouts.map(workout=><button key={workout.id} className="completed-card" onClick={()=>setDetailId(workout.id)}><span className="completed-check">✓</span><span><b>{workout.name}</b><small>{workout.exercises.length} exercises · {workout.exercises.reduce((sum,exercise)=>sum+exercise.sets.filter(set=>set.done).length,0)} sets completed</small></span><em>View ›</em></button>)}</section>}
          ...
          <div className="recent-training-list">{recentHistory.map(item=>item.kind==="strength"?<button className="recent-card" key={`recent-strength-${item.id}`} onClick={()=>setDetailId(item.id)}><span className="recent-date"><b>{new Date(`${item.date}T12:00:00`).getDate()}</b>{new Intl.DateTimeFormat("en",{month:"short"}).format(new Date(`${item.date}T12:00:00`))}</span><span className="recent-main"><b>{item.title}</b><small>{isHybrid?"Strength · ":""}{item.workout.exercises.length} exercises</small></span><span className="chevron">›</span></button>:<button className="recent-card" key={`recent-endurance-${item.id}`} onClick={()=>setEnduranceDetailId(item.id)}><span className="activity-pill">{activityShort(item.session.activityType)}</span><span className="recent-main"><b>{item.title}</b><small>{activityLabel(item.session.activityType)} · {enduranceSummary(item.session)}</small></span><span className="chevron">›</span></button>)}</div>
        </>}

        {tab === "plan" && <>
          <div className="eyebrow">YOUR PROGRAM</div><div className="page-heading"><div><h1>{trainingPreference==="strength"?"Workout templates":trainingPreference==="endurance"?"Endurance plans":"Plan your training"}</h1><p>{trainingPreference==="strength"?"Build once. Train without thinking.":trainingPreference==="endurance"?"Plan the work. Log the progress.":"Strength and endurance, in one diary."}</p></div>{trainingPreference==="strength"&&<button className="round-add" onClick={() => setEditor({id:`template-${Date.now()}`,name:"",focus:"",color:"#409ECE",icon:"◆",exercises:[]})}>＋</button>}</div>
          {showEndurance&&<div className={`plan-type-actions preference-${trainingPreference}`}><button onClick={()=>setEnduranceEditor({mode:"plan"})}><span>END</span><b>Plan endurance session</b><small>Run, bike, swim and more</small></button>{isHybrid&&<button onClick={() => setEditor({id:`template-${Date.now()}`,name:"",focus:"",color:"#409ECE",icon:"◆",exercises:[]})}><span>STR</span><b>New strength template</b><small>Exercises, sets and supersets</small></button>}</div>}
          {showEndurance&&enduranceSessions.some(session=>session.status==="planned")&&<><div className="section-title plan-section-title"><div><span>ENDURANCE</span><h2>Planned sessions</h2></div></div><div className="endurance-plan-list">{enduranceSessions.filter(session=>session.status==="planned").map(session=><button key={session.id} onClick={()=>setEnduranceDetailId(session.id)}><span className="activity-pill">{activityShort(session.activityType)}</span><span><b>{session.title}</b><small>{formatDate(session.date)} · {enduranceSummary(session)}</small></span><em>›</em></button>)}</div></>}
          ...
          {showStrength&&<div className="template-grid">{data.templates.map((template, index) => <article className="template-card" key={template.id}>
        ...
        {tab === "history" && <>
          ...
          {showStrength&&<div className="history-tabs"><button className={effectiveHistoryMode==="sessions"?"active":""} onClick={()=>setHistoryMode("sessions")}>By session</button><button disabled={isHybrid&&historyScope==="endurance"} className={effectiveHistoryMode==="exercises"?"active":""} onClick={()=>setHistoryMode("exercises")}>By exercise</button></div>}
          {showEndurance&&(!isHybrid||historyScope==="all"||historyScope==="endurance")&&<section className="endurance-week-stats"><div><strong>{Number(enduranceThisWeek.distance.toFixed(1))}</strong><span>KM THIS WEEK</span></div><div><strong>{Math.round(enduranceThisWeek.minutes)}</strong><span>MINUTES</span></div><div><strong>{enduranceThisWeek.sessions}</strong><span>SESSIONS</span></div></section>}
          {effectiveHistoryMode==="sessions"?<>{showEndurance&&<label className="history-activity-filter">ACTIVITY<select value={activityFilter} onChange={event=>setActivityFilter(event.target.value)}><option value="all">All activity types</option>{Array.from(new Set(enduranceSessions.map(session=>session.activityType))).map(type=><option key={type} value={type}>{activityLabel(type)}</option>)}</select></label>}<div className="history-list">{visibleHistory.filter(item=>!isHybrid||historyScope==="all"||item.kind===historyScope).filter(item=>item.kind==="strength"||activityFilter==="all"||item.session.activityType===activityFilter).map(item=>item.kind==="strength"?<button key={`strength-${item.id}`} className="history-row" onClick={() => setDetailId(item.id)}><span className="history-date"><b>{new Date(`${item.date}T12:00:00`).getDate()}</b><small>{new Intl.DateTimeFormat("en",{month:"short"}).format(new Date(`${item.date}T12:00:00`))}</small></span><span className="history-info"><small>{isHybrid?"STRENGTH":formatDate(item.date).split(",")[0].toUpperCase()}</small><b>{item.title}</b><em>{item.workout.exercises.length} exercises</em></span><span className="chevron">›</span></button>:<button key={`endurance-${item.id}`} className="history-row endurance-history-row" onClick={()=>setEnduranceDetailId(item.id)}><span className="history-date activity-history-date">{activityShort(item.session.activityType)}</span><span className="history-info"><small>{activityLabel(item.session.activityType).toUpperCase()} · {formatDate(item.date).toUpperCase()}</small><b>{item.title}</b><em>{enduranceSummary(item.session)}</em></span><span className="chevron">›</span></button>)}</div></>:<><label className="search"><span>⌕</span><input value={libraryQuery} onChange={event=>setLibraryQuery(event.target.value)} placeholder="Search exercise history" /></label><div className="library-list">{data.exercises.filter(exercise=>data.workouts.some(workout=>workout.exercises.some(item=>item.exerciseId===exercise.id))&&exercise.name.toLowerCase().includes(libraryQuery.toLowerCase())).map(exercise=>{const sessions=data.workouts.filter(workout=>workout.exercises.some(item=>item.exerciseId===exercise.id));return <button className="library-row" key={exercise.id} onClick={()=>setExerciseHistoryId(exercise.id)}><span className="movement-icon">{exercise.name.split(" ").map(word=>word[0]).slice(0,2).join("")}</span><span><b>{exercise.name}</b><small>{exercise.equipment} · {exercise.group}</small></span><em>{sessions.length} sessions</em><span className="chevron">›</span></button>})}</div></>}
        </>}
      ...
      {scheduleTemplateId && <div className="overlay high-overlay" onMouseDown={()=>setScheduleTemplateId(null)}>
        <section className="sheet schedule-sheet" onMouseDown={event=>event.stopPropagation()}>
          <div className="sheet-handle"/>
          <div className="sheet-title"><div><span>SCHEDULE WORKOUT</span><h2>{data.templates.find(template=>template.id===scheduleTemplateId)?.name}</h2></div><button onClick={()=>setScheduleTemplateId(null)} aria-label="Close">×</button></div>
          ...
          <button className="primary-button schedule-confirm" onClick={scheduleWorkout}>Add to plan <span>→</span></button>
        </section>
      </div>}
      ...
      {active && <div className="workout-screen">
        ...
        <div className="workout-body">{Boolean(active.warmup?.length)&&<section className={`live-warmup ${active.warmup!.every(item=>item.done)?"warmup-complete":""}`}><button className="live-warmup-heading" onClick={()=>setWarmupExpanded(value=>!value)}><span><small>WARM-UP</small><b>{active.warmup!.filter(item=>item.done).length} / {active.warmup!.length} complete</b></span><em>{warmupExpanded?"⌃":"⌄"}</em></button>{warmupExpanded&&<div className="live-warmup-items">{active.warmup!.map((item,index)=><label className={item.done?"done":""} key={item.id}><input type="checkbox" checked={Boolean(item.done)} onChange={event=>setActive({...active,warmup:active.warmup!.map((warmupItem,itemIndex)=>itemIndex===index?{...warmupItem,done:event.target.checked}:warmupItem)})}/><span><b>{item.kind==="exercise"?exerciseName(item.exerciseId||""):item.title?.trim()||"Instructions"}</b>{item.instructions&&<small>{item.instructions}</small>}</span></label>)}</div>}</section>}{active.exercises.map((exercise, exerciseIndex) => { const prev = previousSets(exercise.exerciseId); const groupIndex=[...new Set(active.exercises.map(item=>item.group).filter(Boolean))].indexOf(exercise.group);const allDone=exercise.sets.length>0&&exercise.sets.every(set=>set.done);const minimized=(exercise.skipped||allDone)&&!expandedLiveExercises.has(exerciseIndex);return <article className={`live-exercise ${exercise.group ? `superset-exercise superset-color-${groupIndex%4}` : ""} ${exercise.skipped?"skipped-exercise":""} ${minimized?"minimized-exercise":""}`} key={`${exercise.exerciseId}-${exerciseIndex}`}>
      ...
      {trainingAction&&<div className="overlay high-overlay" onMouseDown={()=>setTrainingAction(null)}><section className="sheet training-type-sheet" onMouseDown={event=>event.stopPropagation()}><div className="sheet-handle"/><div className="sheet-title"><div><span>{trainingAction==="plan"?"PLAN SESSION":"LOG ACTIVITY"}</span><h2>{trainingAction==="plan"?"What are you planning?":"What did you train?"}</h2></div><button onClick={()=>setTrainingAction(null)} aria-label="Close">×</button></div><div className="training-type-options"><button onClick={()=>{setTrainingAction(null);if(trainingAction==="plan")setTab("plan");else{setSelectedDate(today);setPicker(true)}}}><span>STR</span><div><b>Strength</b><small>{trainingAction==="plan"?"Create or schedule a strength workout":"Choose a workout or add exercises as you go"}</small></div><em>›</em></button><button onClick={()=>{const mode=trainingAction;setTrainingAction(null);setEnduranceEditor({mode})}}><span>END</span><div><b>Endurance</b><small>{trainingAction==="plan"?"Plan a run, ride, swim or other session":"Log a run, ride, swim or other activity"}</small></div><em>›</em></button></div></section></div>}
      {enduranceEditor&&<EnduranceSessionSheet mode={enduranceEditor.mode} date={selectedDate} initial={enduranceEditor.initial} onClose={()=>setEnduranceEditor(null)} onSave={saveEnduranceSession}/>}
      {enduranceDetail&&<div className="overlay high-overlay" onMouseDown={()=>setEnduranceDetailId(null)}><section className="sheet endurance-detail-sheet" onMouseDown={event=>event.stopPropagation()}><div className="sheet-handle"/><div className="sheet-title"><div><span>{activityLabel(enduranceDetail.activityType).toUpperCase()} · {enduranceDetail.status.toUpperCase()}</span><h2>{enduranceDetail.title}</h2></div><button onClick={()=>setEnduranceDetailId(null)} aria-label="Close">×</button></div><div className="endurance-detail-hero"><span className="activity-pill">{activityShort(enduranceDetail.activityType)}</span><div><b>{formatDate(enduranceDetail.date)}</b><small>{enduranceSummary(enduranceDetail)}</small></div></div>{enduranceDetail.targetRpe!=null&&enduranceDetail.status==="planned"&&<p className="endurance-detail-metric"><span>TARGET RPE</span><b>{enduranceDetail.targetRpe}/10</b></p>}{enduranceDetail.status==="completed"&&<div className="endurance-detail-metrics">{enduranceDetail.rpe!=null&&<p><span>RPE</span><b>{enduranceDetail.rpe}/10</b></p>}{enduranceDetail.averageHeartRate!=null&&<p><span>AVG HR</span><b>{enduranceDetail.averageHeartRate} bpm</b></p>}{enduranceDetail.elevationGainMetres!=null&&<p><span>ELEVATION</span><b>{enduranceDetail.elevationGainMetres} m</b></p>}</div>}{enduranceDetail.blocks.length>0&&<div className="endurance-detail-blocks"><span>SESSION STEPS</span><EnduranceBlockDisplay session={enduranceDetail} detail/></div>}{enduranceDetail.notes&&<p className="detail-note">{enduranceDetail.notes}</p>}<div className="endurance-detail-actions">{enduranceDetail.status==="planned"&&<button className="primary-button" onClick={()=>{setEnduranceDetailId(null);setEnduranceEditor({mode:"log",initial:enduranceDetail})}}>Complete session <span>→</span></button>}<button onClick={()=>{setEnduranceDetailId(null);setEnduranceEditor({mode:enduranceDetail.status==="completed"?"log":"plan",initial:enduranceDetail})}}>Edit {enduranceDetail.status==="completed"?"activity":"plan"}</button><button className="delete-workout-button" onClick={()=>setDeleteEnduranceId(enduranceDetail.id)}>Delete session</button></div></section></div>}
      ...
      {active && liveEditIndex !== null && active.exercises[liveEditIndex] && <div className="overlay high-overlay" onMouseDown={()=>setLiveEditIndex(null)}>
