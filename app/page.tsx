...
import {newBlankWorkout,newTemplateWorkout} from "@/lib/setra/new-workout";
...
export default function Home() {
  ...
  useEffect(()=>{if(!syncRecoveryOpen||!diaryService||!trainingService)return;let cancelled=false;setRecoveryLoadState("loading");setRecoverySnapshot(null);Promise.all([diaryService.load(),trainingService.loadEndurance(),trainingService.loadEnduranceTemplates()]).then(([strength,sessions,templates])=>{if(!cancelled){setRecoverySnapshot({strength,sessions,templates});setRecoveryLoadState("ready")}}).catch(()=>{if(!cancelled){setRecoverySnapshot(null);setRecoveryLoadState("error")}});return()=>{cancelled=true}},[syncRecoveryOpen,diaryService,trainingService]);
  ...
  function discardSyncIssue(change:PendingDiaryChange){const backup=backupPendingDiaryChange(change,user?.id);if(!backup.ok){setCloudMessage("Setra could not create a recoverable backup, so the device copy was kept.");return}const removed=removePendingDiaryChange(change.key,change.operationId,user?.id);if(!removed.ok){setCloudMessage("The device copy could not be removed and remains available for recovery.");return}setSyncIssues(current=>current.filter(item=>item.operationId!==change.operationId));setDiscardRecoveryId(null);setSyncRecoveryOpen(false);window.location.reload()}
  function saveSyncIssueAsCopy(change:PendingDiaryChange){const suffix=`recovered-${Date.now()}`;let queued:{ok:boolean;error?:string}|null=null;let apply:()=>void=()=>{};if(change.kind==="save_workout"){const workout={...change.payload.workout,id:`${change.payload.workout.id}-${suffix}`,name:`${change.payload.workout.name} (recovered copy)`};queued=queuePendingDiaryChange({key:`workout:${workout.id}`,kind:"save_workout",payload:{workout,status:change.payload.status}},user?.id);apply=()=>setData(current=>({...current,workouts:change.payload.status==="completed"?[workout,...current.workouts]:current.workouts}))}else if(change.kind==="save_strength_template"){const template={...change.payload.template,id:`${change.payload.template.id}-${suffix}`,name:`${change.payload.template.name} (recovered copy)`};queued=queuePendingDiaryChange({key:`strength-template:${template.id}`,kind:"save_strength_template",payload:{template}},user?.id);apply=()=>setData(current=>({...current,templates:[template,...current.templates]}))}else if(change.kind==="save_endurance_session"){const session={...change.payload.session,id:`${change.payload.session.id}-${suffix}`,title:`${change.payload.session.title} (recovered copy)`,plannedSessionId:undefined};queued=queuePendingDiaryChange({key:`endurance-session:${session.id}`,kind:"save_endurance_session",payload:{session}},user?.id);apply=()=>setEnduranceSessions(current=>[session,...current])}else if(change.kind==="save_endurance_template"){const template={...change.payload.template,id:`${change.payload.template.id}-${suffix}`,title:`${change.payload.template.title} (recovered copy)`};queued=queuePendingDiaryChange({key:`endurance-template:${template.id}`,kind:"save_endurance_template",payload:{template}},user?.id);apply=()=>setEnduranceTemplates(current=>[template,...current])}else return;if(!queued?.ok){setCloudMessage("The separate copy could not be stored. Your original device change is still safe.");return}apply();const removed=removePendingDiaryChange(change.key,change.operationId,user?.id);if(!removed.ok){setCloudMessage("The separate copy was saved, and the original remains available until Setra can safely clear it.");return}setSyncIssues(current=>current.filter(item=>item.operationId!==change.operationId));setSyncRecoveryOpen(false);window.dispatchEvent(new Event("online"))}
  ...
  function startWorkout(template: Template, workoutDate = today) {
    ...
    setActive(newTemplateWorkout(template,workoutDate,data.workouts.length+1,localTime(),template.exercises.map(liveExerciseFromTemplate)));
    setPicker(false);
  }
  function startBlankWorkout() {
    ...
    setExpandedLiveExercises(new Set());
    setEditingWorkoutId(null);
    setActive(newBlankWorkout(selectedDate,data.workouts.length+1,localTime()));
    setPicker(false);
  }
  function saveDraft() {
    if (!active) return;
    ...
    <main className="app-shell" data-light-accent={contrastColour(appColour)==="#0F172A"} data-theme={resolvedAppearance} style={createSetraTheme(appColour,resolvedAppearance,textScale)}>
      ...
      <section className={`content content-${tab}`}>
        ...
        {tab === "plan" && <>
          ...
          {isHybrid&&<div className="section-title plan-section-title"><div><span>STRENGTH</span><h2>Workout templates</h2></div><button onClick={() => setEditor({id:`template-${Date.now()}`,name:"",focus:"",color:"#409ECE",icon:"◆",exercises:[]})}>New template</button></div>}
