...
import {analytics,trackWeeklyReturnOnce} from "@/lib/analytics/events";
...
export default function Home() {
  ...
  useEffect(()=>{
    ...
    const replay=async()=>{
      ...
      }finally{replaying=false;refreshIssues()}
    };
  ...
  useEffect(()=>{
    ...
    retryCloud(()=>Promise.all([diaryService.load(),diaryService.loadDraft(),diaryService.loadProfile(),diaryService.loadWriteVersions()])).then(([cloud,draft,profile,writeVersions])=>{
      ...
      if(reconciledSchedule.length!==cloud.scheduled.length){cloud.scheduled=reconciledSchedule;const key="schedule:current";const queued=queuePendingDiaryChange({key,kind:"replace_schedule",payload:{items:reconciledSchedule}},user?.id);void diaryService.replaceSchedule(reconciledSchedule,queued).then(version=>removePendingDiaryChange(key,queued.operationId,user?.id,version))}
  ...
  useEffect(()=>{if(!syncRecoveryOpen||!diaryService||!trainingService)return;let cancelled=false;Promise.all([diaryService.load(),trainingService.loadEndurance(),trainingService.loadEnduranceTemplates()]).then(([strength,sessions,templates])=>{if(!cancelled)setRecoverySnapshot({strength,sessions,templates})}).catch(()=>{if(!cancelled)setRecoverySnapshot(null)});return()=>{cancelled=true}},[syncRecoveryOpen,diaryService,trainingService]);

  function runCloud<T>(action:(service:DiaryService)=>Promise<T>){
    if(!diaryService)return;
    setCloudState("loading");return retryCloud(()=>action(diaryService)).then(version=>{setCloudState("synced");setCloudMessage("");return version}).catch(error=>{analytics.track("save_failed",{outcome:"failed"});setCloudState("error");setCloudMessage(userWriteErrorMessage(error));return null});
  }
  function navigateTab(next:Tab){setTab(next);const url=next==="today"?"/":`/?tab=${next}`;window.history.pushState({},"",url)}
  function persistSchedule(items:AppData["scheduled"]){const key="schedule:current";const queued=queuePendingDiaryChange({key,kind:"replace_schedule",payload:{items}},user?.id);return runCloud(service=>service.replaceSchedule(items,queued))?.then(version=>{if(version!=null)removePendingDiaryChange(key,queued.operationId,user?.id,version);else if(queued.ok)setCloudMessage("Schedule changes are saved on this device and waiting to sync.");return version!=null})}
  function runTrainingCloud(action:(service:TrainingSessionService)=>Promise<number>){if(!trainingService)return;setCloudState("loading");return retryCloud(()=>action(trainingService)).then(version=>{setCloudState("synced");setCloudMessage("");return version}).catch(error=>{analytics.track("save_failed",{outcome:"failed"});setCloudState("error");setCloudMessage(userWriteErrorMessage(error));return null})}
  ...
  function saveEnduranceValue(value:EnduranceSession|EnduranceTemplate){
    ...
    if(newlyCompleted){analytics.track("training_mode_used",{modality:"endurance",source:session.templateId?"template":"manual"});if(!enduranceSessions.some(item=>item.status==="completed"))analytics.track("first_session_completed",{modality:"endurance",source:session.templateId?"template":"manual"})}
    ...
    runTrainingCloud(service=>service.save(session,queued))?.then(version=>{if(version!=null){removePendingDiaryChange(key,queued.operationId,user?.id,version);window.dispatchEvent(new Event("setra-training-changed"))}});
  }
  ...
  const serverLabel=(change:PendingDiaryChange)=>{if(!recoverySnapshot)return "Loading cloud version…";if(change.kind==="save_workout"||change.kind==="delete_workout")return recoverySnapshot.strength.workouts.find(item=>item.id===(change.kind==="save_workout"?change.payload.workout.id:change.payload.clientId))?.name||"No cloud workout";if(change.kind==="save_strength_template"||change.kind==="delete_strength_template")return recoverySnapshot.strength.templates.find(item=>item.id===(change.kind==="save_strength_template"?change.payload.template.id:change.payload.clientId))?.name||"No cloud template";if(change.kind==="save_endurance_session"||change.kind==="delete_endurance_session")return recoverySnapshot.sessions.find(item=>item.id===(change.kind==="save_endurance_session"?change.payload.session.id:change.payload.clientId))?.title||"No cloud session";if(change.kind==="save_endurance_template"||change.kind==="delete_endurance_template")return recoverySnapshot.templates.find(item=>item.id===(change.kind==="save_endurance_template"?change.payload.template.id:change.payload.clientId))?.title||"No cloud template";return `${recoverySnapshot.strength.scheduled.length} scheduled items`};
  ...
  function discardSyncIssue(change:PendingDiaryChange){removePendingDiaryChange(change.key,change.operationId,user?.id);setSyncIssues(current=>current.filter(item=>item.operationId!==change.operationId));setSyncRecoveryOpen(false);window.location.reload()}
  function saveSyncIssueAsCopy(change:PendingDiaryChange){const suffix=`recovered-${Date.now()}`;if(change.kind==="save_workout"){const workout={...change.payload.workout,id:`${change.payload.workout.id}-${suffix}`,name:`${change.payload.workout.name} (recovered copy)`};queuePendingDiaryChange({key:`workout:${workout.id}`,kind:"save_workout",payload:{workout,status:change.payload.status}},user?.id);setData(current=>({...current,workouts:change.payload.status==="completed"?[workout,...current.workouts]:current.workouts}))}else if(change.kind==="save_strength_template"){const template={...change.payload.template,id:`${change.payload.template.id}-${suffix}`,name:`${change.payload.template.name} (recovered copy)`};queuePendingDiaryChange({key:`strength-template:${template.id}`,kind:"save_strength_template",payload:{template}},user?.id);setData(current=>({...current,templates:[template,...current.templates]}))}else if(change.kind==="save_endurance_session"){const session={...change.payload.session,id:`${change.payload.session.id}-${suffix}`,title:`${change.payload.session.title} (recovered copy)`,plannedSessionId:undefined};queuePendingDiaryChange({key:`endurance-session:${session.id}`,kind:"save_endurance_session",payload:{session}},user?.id);setEnduranceSessions(current=>[session,...current])}else if(change.kind==="save_endurance_template"){const template={...change.payload.template,id:`${change.payload.template.id}-${suffix}`,title:`${change.payload.template.title} (recovered copy)`};queuePendingDiaryChange({key:`endurance-template:${template.id}`,kind:"save_endurance_template",payload:{template}},user?.id);setEnduranceTemplates(current=>[template,...current])}else return;removePendingDiaryChange(change.key,change.operationId,user?.id);setSyncIssues(current=>current.filter(item=>item.operationId!==change.operationId));setSyncRecoveryOpen(false);window.dispatchEvent(new Event("online"))}
  ...
  const effectiveHistoryMode=trainingPreference==="endurance"&&historyMode==="exercises"?"sessions":historyMode;
  const progressWeeks=useMemo(()=>Array.from({length:6},(_,index)=>{const anchor=new Date(`${today}T12:00:00`);anchor.setDate(anchor.getDate()-(5-index)*7);const dates=weekDateKeys(localDateKey(anchor),weekStartsOn);const strength=data.workouts.filter(workout=>workout.date>=dates[0]&&workout.date<=dates[6]&&hasCompletedStrengthWork(workout));const endurance=enduranceSessions.filter(session=>session.status==="completed"&&session.date>=dates[0]&&session.date<=dates[6]);const knownMinutes=[...strength.map(item=>item.duration),...endurance.map(item=>item.durationMinutes)].filter((value):value is number=>typeof value==="number"&&value>0);return{key:dates[0],label:new Intl.DateTimeFormat("en-AU",{day:"numeric",month:"short"}).format(new Date(`${dates[0]}T12:00:00`)),sessions:strength.length+endurance.length,strength:strength.length,endurance:endurance.length,minutes:knownMinutes.reduce((sum,value)=>sum+value,0),hasTime:knownMinutes.length>0,distance:endurance.reduce((sum,item)=>sum+(item.distanceKm||0),0)}}),[data.workouts,enduranceSessions,today,weekStartsOn]);
  const strengthTrends=useMemo(()=>data.exercises.flatMap(exercise=>{const records=data.workouts.flatMap(workout=>workout.exercises.filter(item=>item.exerciseId===exercise.id&&!item.skipped&&(item.loadMode==null||item.loadMode==="kg")).map(item=>({date:workout.date,max:Math.max(0,...completedSets(item).map(set=>Number(set.weight)||0))}))).filter(item=>item.max>0).sort((a,b)=>a.date.localeCompare(b.date));if(records.length<2)return[];const first=records[0],last=records.at(-1)!;return[{exercise,first,last,change:last.max-first.max}] }).sort((a,b)=>Math.abs(b.change)-Math.abs(a.change)).slice(0,3),[data.exercises,data.workouts]);
  ...
  const strengthThisWeek=useMemo(()=>{const sessions=data.workouts.filter(workout=>workout.date>=currentWeekDates[0]&&workout.date<=currentWeekDates[6]&&hasCompletedStrengthWork(workout));return {sessions:sessions.length,minutes:sessions.reduce((sum,workout)=>sum+(workout.duration||0),0)}},[data.workouts,currentWeekDates]);
  ...
  function saveDraft() {
    ...
    runCloud(service=>service.saveWorkout({...active,updatedAt:local.updatedAt},"in_progress",queued))?.then(version=>{if(version!=null)removePendingDiaryChange(key,queued.operationId,user?.id,version);setDraftSaveState(version!=null?"saved":local.ok&&queued.ok?"pending":"error")});
  ...
  function saveWorkout(timing?:{startedAt:string;endedAt:string}) {
    ...
    if (!editingWorkoutId) {
      setCompletionReceipt(completed);setCompletionReceiptPbs(showPbPopup?records:[]);
      setNewTemplateName("");
    ...
    const workoutKey=`workout:${completed.id}`;const queuedWorkout=queuePendingDiaryChange({key:workoutKey,kind:"save_workout",payload:{workout:completed,status:"completed"}},user?.id);
    if(!queuedWorkout.ok){setCompletionSaving(false);setCloudState("error");setCloudMessage("The completed workout could not be saved on this device. It remains open so you can try again.");return}
  ...
  function saveSessionAsTemplate() {
    ...
    const key=`strength-template:${template.id}`;setData(current=>({...current,templates:[...current.templates,template]}));const queued=queuePendingDiaryChange({key,kind:"save_strength_template",payload:{template}},user?.id);runCloud(service=>service.saveTemplate(template,queued))?.then(version=>{if(version!=null)removePendingDiaryChange(key,queued.operationId,user?.id,version)});setSessionTemplateSaved(true);
  }
  ...
  function scheduleEnduranceWorkout(){
    ...
    setEnduranceSessions(current=>[...additions,...current].sort((a,b)=>b.date.localeCompare(a.date)));additions.forEach(session=>{const key=`endurance-session:${session.id}`;const queued=queuePendingDiaryChange({key,kind:"save_endurance_session",payload:{session}},user?.id);runTrainingCloud(service=>service.save(session,queued))?.then(version=>{if(version!=null)removePendingDiaryChange(key,queued.operationId,user?.id,version)})});setSelectedDate(scheduleDate);setScheduleEnduranceTemplateId(null);
  }
    ...
    <main className="app-shell" data-light-accent={contrastColour(appColour)==="#0F172A"} data-theme={resolvedAppearance} style={createSetraTheme(appColour,resolvedAppearance,textScale)}>
      ...
      <section className={`content content-${tab}`}>
        ...
        {tab === "plan" && <>
          ...
          {isHybrid&&<div className="section-title plan-section-title"><div><span>STRENGTH</span><h2>Workout templates</h2></div><button onClick={() => setEditor({id:`template-${Date.now()}`,name:"",focus:"",color:"#409ECE",icon:"◆",exercises:[]})}>New</button></div>}
        ...
        {tab === "history" && <>
          ...
          {isHybrid&&<div className="history-scope">{([['all','All training'],['strength','Strength'],['endurance','Endurance']] as const).map(([value,label])=><button key={value} className={historyScope===value?"active":""} onClick={()=>{setHistoryScope(value);if(value==="endurance")setHistoryMode("sessions")}}>{label}</button>)}</div>}
          ...
          {showEndurance&&(!isHybrid||historyScope==="endurance")&&<section className="endurance-week-stats"><div><strong>{Number(enduranceThisWeek.distance.toFixed(1))}</strong><span>KM RECORDED</span></div><div><strong>{Math.round(enduranceThisWeek.minutes)}</strong><span>MINUTES RECORDED</span></div><div><strong>{enduranceThisWeek.sessions}</strong><span>SESSIONS RECORDED</span></div></section>}
          {isHybrid&&historyScope==="all"&&<section className="endurance-week-stats"><div><strong>{strengthThisWeek.sessions+enduranceThisWeek.sessions}</strong><span>ALL SESSIONS</span></div><div><strong>{Math.round(strengthThisWeek.minutes+enduranceThisWeek.minutes)}</strong><span>TRAINING MIN</span></div><div><strong>{Number(enduranceThisWeek.distance.toFixed(1))}</strong><span>ENDURANCE KM</span></div></section>}
          {effectiveHistoryMode==="progress"?<section className="progress-overview" aria-label="Training progress"><div className="progress-section-heading"><span>WEEKLY CONSISTENCY</span><h2>The work adds up.</h2><p>Completed training only. Missing duration or distance stays unknown rather than becoming zero.</p></div><div className="weekly-progress-bars">{progressWeeks.map(week=><div key={week.key} title={`${week.sessions} ${week.sessions===1?"session":"sessions"}`}><span style={{height:`${Math.max(8,Math.min(100,week.sessions*22))}%`}}/><b>{week.sessions}</b><small>{week.label}</small></div>)}</div>{showStrength&&<div className="progress-group"><span>STRENGTH TRENDS</span>{strengthTrends.length?strengthTrends.map(item=><button key={item.exercise.id} onClick={()=>setExerciseHistoryId(item.exercise.id)}><b>{item.exercise.name}</b><small>{formatLoad(item.first.max,preferredUnit)} → {formatLoad(item.last.max,preferredUnit)}</small><em>{item.change===0?"Stable":`${item.change>0?"+":""}${formatLoad(item.change,preferredUnit)}`}</em></button>):<p>Complete the same weighted exercise in at least two sessions to see a comparable trend. Band, bodyweight and numeric loads are kept separate.</p>}</div>}{showEndurance&&<div className="progress-group"><span>ENDURANCE TOTALS</span>{enduranceTrends.length?enduranceTrends.map(item=><div key={item.activityType}><ActivityIcon type={item.activityType}/><p><b>{activityLabel(item.activityType)}</b><small>{item.sessions} {item.sessions===1?"session":"sessions"}</small></p><em>{[item.hasDistance?`${Number(item.distance.toFixed(1))} km`:"distance not recorded",item.hasTime?formatMinutes(item.minutes):"time not recorded"].join(" · ")}</em></div>):<p>Complete an endurance activity to begin building distance and time trends.</p>}</div>}</section>:effectiveHistoryMode==="sessions"?<>{showEndurance&&(!isHybrid||historyScope!=="strength")&&<label className="history-activity-filter">ACTIVITY<select value={activityFilter} onChange={event=>setActivityFilter(event.target.value)}><option value="all">All activity types</option>{Array.from(new Set(enduranceSessions.map(session=>session.activityType))).map(type=><option key={type} value={type}>{activityLabel(type)}</option>)}</select></label>}{filteredSessionHistory.length?<div className="history-list">{filteredSessionHistory.map(item=>item.kind==="strength"?<button key={`strength-${item.id}`} className="history-row" onClick={() => setDetailId(item.id)}><span className="history-date"><b>{new Date(`${item.date}T12:00:00`).getDate()}</b><small>{new Intl.DateTimeFormat("en",{month:"short"}).format(new Date(`${item.date}T12:00:00`))}</small></span><span className="history-info"><small>{isHybrid?"STRENGTH":formatDate(item.date).split(",")[0].toUpperCase()}</small><b>{item.title}</b><em>{strengthCompletion(item.workout).status==="partial"?"Partial · ":""}{completedExercises(item.workout).length} {completedExercises(item.workout).length===1?"exercise":"exercises"}</em></span><span className="chevron">›</span></button>:<button key={`endurance-${item.id}`} className="history-row endurance-history-row" onClick={()=>setEnduranceDetailId(item.id)}><ActivityIcon type={item.session.activityType}/><span className="history-info"><small>{activityLabel(item.session.activityType).toUpperCase()} · {formatDate(item.date).toUpperCase()}</small><b>{item.title}</b><em>{enduranceSummary(item.session)}</em></span><span className="chevron">›</span></button>)}</div>:<div className="filtered-empty"><b>No matching activities</b><p>Try another training type or show all activity types.</p><button onClick={()=>{setHistoryScope("all");setActivityFilter("all")}}>Clear filters</button></div>}</>:<><label className="search"><span>⌕</span><input value={libraryQuery} onChange={event=>setLibraryQuery(event.target.value)} placeholder="Search exercise history" /></label>{filteredExerciseHistory.length?<div className="library-list">{filteredExerciseHistory.map(exercise=>{const sessions=data.workouts.filter(workout=>workout.exercises.some(item=>item.exerciseId===exercise.id&&completedSets(item).length>0));return <button className="library-row" key={exercise.id} onClick={()=>setExerciseHistoryId(exercise.id)}><span className="movement-icon">{exercise.name.split(" ").map(word=>word[0]).slice(0,2).join("")}</span><span><b>{exercise.name}</b><small>{exercise.equipment} · {exercise.group}</small></span><em>{sessions.length} {sessions.length===1?"session":"sessions"}</em><span className="chevron">›</span></button>})}</div>:<div className="filtered-empty"><b>No matching exercise history</b><p>Try another exercise name.</p><button onClick={()=>setLibraryQuery("")}>Clear search</button></div>}</>}
        </>}
      ...
      {syncRecoveryOpen&&<div className="overlay high-overlay sync-recovery-overlay"><section className="sync-recovery-dialog" role="dialog" aria-modal="true" aria-labelledby="sync-recovery-title"><header><div><span>SYNC RECOVERY</span><h2 id="sync-recovery-title">Review saved changes</h2><p>Your device copy and the latest cloud copy are both preserved.</p></div><button onClick={()=>setSyncRecoveryOpen(false)} aria-label="Close sync recovery">×</button></header><div className="sync-recovery-list">{syncIssues.map(change=><article key={change.operationId}><b>{pendingLabel(change)}</b><small>{change.failure?.message}</small><dl><div><dt>THIS DEVICE</dt><dd>{pendingLabel(change)}</dd></div><div><dt>CLOUD</dt><dd>{serverLabel(change)}</dd></div></dl><div><button onClick={()=>retrySyncIssue(change)}>Try again</button><button onClick={()=>discardSyncIssue(change)}>Use cloud copy</button>{change.kind.startsWith("save_")&&<button className="primary-button" onClick={()=>saveSyncIssueAsCopy(change)}>Save separate copy</button>}</div></article>)}</div>{syncIssues.length===0&&<p className="sync-recovery-empty">No changes currently need review.</p>}</section></div>}
