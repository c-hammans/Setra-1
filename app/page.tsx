    ...
    <main className="app-shell" data-light-accent={contrastColour(appColour)==="#0F172A"} data-theme={resolvedAppearance} style={createSetraTheme(appColour,resolvedAppearance,textScale)}>
      ...
      <section className={`content content-${tab}`}>
        ...
        {tab === "plan" && <>
          ...
          {showEndurance&&<div className={`plan-type-actions preference-${trainingPreference}`}><button onClick={()=>setEnduranceEditor({mode:"plan"})}><b>Plan endurance session</b><small>Run, bike, swim and more</small></button>{isHybrid&&<button onClick={() => setEditor({id:`template-${Date.now()}`,name:"",focus:"",color:"#409ECE",icon:"◆",exercises:[]})}><b>New strength template</b><small>Exercises, sets and supersets</small></button>}</div>}
          ...
          {showStrength&&<div className="template-grid">{data.templates.map(template => <article className="template-card" key={template.id}>
            <div className="template-copy"><h2>{template.name}</h2>{template.focus&&<span className="template-focus">{template.focus}</span>}<p>{template.exercises.length} exercises · {template.exercises.reduce((sum,item)=>sum+item.sets,0)} sets</p></div>
