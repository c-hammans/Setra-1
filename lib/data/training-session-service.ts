...
export class TrainingSessionService{
  ...
  async save(session:EnduranceSession,revision=Date.now()*1000){
    ...
    return runOrderedWrite(`endurance-session:${session.id}`,async()=>{const {error}=await this.supabase.rpc("save_endurance_session_revisioned",{p_session:payload,p_revision:revision});if(error)throw error});
  }

  async saveTemplate(template:EnduranceTemplate,revision=Date.now()*1000){
    return runOrderedWrite(`endurance-template:${template.id}`,async()=>{const {error}=await this.supabase.rpc("save_endurance_template_revisioned",{p_template:{...template,plannedDistanceMetres:metres(template.plannedDistanceKm)},p_revision:revision});if(error)throw error});
  }

  async deleteTemplate(clientId:string,revision=Date.now()*1000){return runOrderedWrite(`endurance-template:${clientId}`,async()=>{const {error}=await this.supabase.rpc("delete_endurance_template_revisioned",{p_client_id:clientId,p_revision:revision});if(error)throw error})}

  async delete(clientId:string,revision=Date.now()*1000){return runOrderedWrite(`endurance-session:${clientId}`,async()=>{const {error}=await this.supabase.rpc("delete_endurance_session_revisioned",{p_client_id:clientId,p_revision:revision,p_today:localDateKey()});if(error)throw error})}
}
