...
export class TrainingSessionService{
  ...
  async save(session:EnduranceSession){
    const payload={...session,plannedDistanceMetres:metres(session.plannedDistanceKm),startedAt:isoFor(session.date,session.startedAt),endedAt:isoFor(session.date,session.endedAt),durationSeconds:seconds(session.durationMinutes),distanceMetres:metres(session.distanceKm),completedAt:session.status==="completed"?(session.completedAt||new Date().toISOString()):undefined};
    const {error}=await this.supabase.rpc("save_endurance_session",{p_session:payload});if(error)throw error;
  }

  async saveTemplate(template:EnduranceTemplate){
    const {error}=await this.supabase.rpc("save_endurance_template",{p_template:{...template,plannedDistanceMetres:metres(template.plannedDistanceKm)}});if(error)throw error;
  }
