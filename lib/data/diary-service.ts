...
import type { AppData, Exercise, ScheduledWorkout, Template, Workout } from "@/lib/setra/types";
...
export class DiaryService {
  ...
  async load():Promise<AppData>{
    const [exerciseResult,templateResult,scheduleResult,workoutResult]=await Promise.all([
      ...
      this.supabase.from("workout_templates").select("*,template_supersets(*),template_exercises(*)").order("created_at"),
      this.supabase.from("scheduled_workouts").select("*,workout_templates(client_id)"),
      this.supabase.from("workouts").select("*,workout_templates(client_id),workout_exercises(*,workout_sets(*))").eq("status","completed").order("workout_date",{ascending:false}),
    ]);
  ...
  async loadDraft():Promise<Workout|null>{
    const {data,error}=await this.supabase.from("workouts").select("*,workout_templates(client_id),workout_exercises(*,workout_sets(*))").eq("status","in_progress").order("updated_at",{ascending:false}).limit(1).maybeSingle();
  ...
  private mapWorkout(row:AnyRow):Workout{
    ...
    return {id:String(row.client_id||row.id),templateId:template?.client_id?String(template.client_id):undefined,name:String(row.name),date:String(row.workout_date),startedAt:row.started_at?String(row.started_at).slice(0,5):"",endedAt:row.ended_at?String(row.ended_at).slice(0,5):undefined,duration:0,note:String(row.notes||""),exercises:exercises.map(item=>({exerciseId:String(item.exercise_id),group:item.superset_key?String(item.superset_key):undefined,note:String(item.notes||""),planNote:String(item.planning_notes||""),repTarget:item.rep_target?String(item.rep_target):undefined,loadMode:item.load_mode as "kg"|"text",skipped:Boolean(item.skipped),sets:(item.workout_sets as AnyRow[]||[]).sort((a,b)=>Number(a.set_number)-Number(b.set_number)).map(set=>({reps:String(set.reps||""),weight:String(set.load_text??set.weight??""),rpe:String(set.rpe||""),done:Boolean(set.completed),note:String(set.notes||"")}))}))};
  }
  ...
  async saveWorkout(workout:Workout,status:"in_progress"|"completed"="completed"){
    ...
    for(const [position,item] of workout.exercises.entries()){
      const {data:exerciseRow,error:exerciseError}=await this.supabase.from("workout_exercises").insert({user_id:this.userId,workout_id:workoutId,exercise_id:item.exerciseId,position,superset_key:item.group||null,superset_name:item.group?workout.supersetNames?.[item.group]||null:null,notes:item.note||"",planning_notes:item.planNote||"",rep_target:item.repTarget||null,load_mode:item.loadMode||"kg",skipped:Boolean(item.skipped)}).select("id").single();if(exerciseError)throw exerciseError;
      if(item.sets.length){const {error:setError}=await this.supabase.from("workout_sets").insert(item.sets.map((set,index)=>({user_id:this.userId,workout_exercise_id:exerciseRow.id,set_number:index+1,weight:item.loadMode==="text"?null:Number(set.weight)||null,load_text:item.loadMode==="text"?set.weight||null:null,reps:set.reps||null,rpe:Number(set.rpe)||null,notes:set.note||"",completed:set.done})));if(setError)throw setError;}
    }
  }
