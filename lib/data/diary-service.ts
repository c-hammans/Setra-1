...
export class DiaryService {
  ...
  async loadProfile():Promise<{displayName:string;bodyWeightKg:number|null;preferredUnit:"kg"|"lb";appColour:string;appearanceMode:AppearanceMode;textScale:TextScale;trainingPreference:TrainingPreference;showWorkoutTimingPopup:boolean;showPbPopup:boolean;weekStartsOn:WeekdayIndex;lastWeeklyPreviewWeekStart:string|null}>{
    const {data,error}=await this.supabase.from("profiles").select("display_name,body_weight_kg,preferred_unit,app_colour,appearance_mode,text_scale,training_preference,show_workout_timing_popup,show_pb_popup,week_starts_on,last_weekly_preview_week_start").eq("id",this.userId).single();
    if(error){
      if(error.code==="42703"||error.code==="PGRST204"){const fallback=await this.supabase.from("profiles").select("display_name,preferred_unit,app_colour,appearance_mode,text_scale,training_preference,show_workout_timing_popup,show_pb_popup,week_starts_on,last_weekly_preview_week_start").eq("id",this.userId).single();if(fallback.error)throw fallback.error;const previous=fallback.data;return {displayName:previous.display_name||"",bodyWeightKg:null,preferredUnit:previous.preferred_unit==="lb"?"lb":"kg",appColour:previous.app_colour||"#409ECE",appearanceMode:previous.appearance_mode==="light"||previous.appearance_mode==="dark"?previous.appearance_mode:"system",textScale:[1,1.1,1.2,1.3].includes(Number(previous.text_scale))?Number(previous.text_scale) as TextScale:1,trainingPreference:["strength","endurance","hybrid"].includes(previous.training_preference)?previous.training_preference as TrainingPreference:"strength",showWorkoutTimingPopup:previous.show_workout_timing_popup!==false,showPbPopup:previous.show_pb_popup!==false,weekStartsOn:Number.isInteger(previous.week_starts_on)&&previous.week_starts_on>=0&&previous.week_starts_on<=6?previous.week_starts_on as WeekdayIndex:1,lastWeeklyPreviewWeekStart:previous.last_weekly_preview_week_start||null}}
      throw error;
    }
    return {displayName:data.display_name||"",bodyWeightKg:data.body_weight_kg==null?null:Number(data.body_weight_kg),preferredUnit:data.preferred_unit==="lb"?"lb":"kg",appColour:data.app_colour||"#409ECE",appearanceMode:data.appearance_mode==="light"||data.appearance_mode==="dark"?data.appearance_mode:"system",textScale:[1,1.1,1.2,1.3].includes(Number(data.text_scale))?Number(data.text_scale) as TextScale:1,trainingPreference:["strength","endurance","hybrid"].includes(data.training_preference)?data.training_preference as TrainingPreference:"strength",showWorkoutTimingPopup:data.show_workout_timing_popup!==false,showPbPopup:data.show_pb_popup!==false,weekStartsOn:Number.isInteger(data.week_starts_on)&&data.week_starts_on>=0&&data.week_starts_on<=6?data.week_starts_on as WeekdayIndex:1,lastWeeklyPreviewWeekStart:data.last_weekly_preview_week_start||null};
  }
