...
const change=(operationId:string,revision:number,name:string):PendingDiaryChange=>({key:"workout:w",operationId,revision,updatedAt:new Date(revision).toISOString(),kind:"save_workout",payload:{workout:workout(name),status:"completed"}});
...
test("a pending delete cannot be resurrected by cloud refresh",()=>{
  const deleted:PendingDiaryChange={key:"workout:w",operationId:"d",revision:3,updatedAt:new Date().toISOString(),kind:"delete_workout",payload:{clientId:"w"}};
