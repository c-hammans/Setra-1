...
export class ProfileService{
  ...
  async load():Promise<ProfileSettings>{
    const {data,error}=await this.supabase.from("profiles").select("display_name,full_name,date_of_birth,preferred_unit,training_goal,experience_level,training_preference,app_colour,appearance_mode,text_scale,show_workout_timing_popup,show_pb_popup,week_starts_on,last_weekly_preview_week_start").eq("id",this.userId).single();
    if(error){
